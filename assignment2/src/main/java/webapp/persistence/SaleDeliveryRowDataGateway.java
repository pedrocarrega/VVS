...
public class SaleDeliveryRowDataGateway {
	...
	private void fillAttributes(int sale_id, int customerVat, int address_id) {
		this.sale_id = sale_id;
		this.customer_vat = customerVat;
		this.addr_id = address_id;
	}
	 ...
	 * Remove sale part
	 */
	private static final String REMOVE_DELIVERY_BY_VAT = "DELETE FROM saledelivery where customer_vat = ?";
	
	public static void removeDeliveryByVat (int vat) throws PersistenceException {
		try (PreparedStatement statement = DataSource.INSTANCE.prepare(REMOVE_DELIVERY_BY_VAT)){
			// set statement arguments
			statement.setInt(1, vat);
			//execute SQL
			statement.executeUpdate();
		} catch (SQLException e) {
			throw new PersistenceException("Internal error removing delivery from " + vat + ".", e);
		}
	}
