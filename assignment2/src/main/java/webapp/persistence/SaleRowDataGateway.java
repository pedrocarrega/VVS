...
public class SaleRowDataGateway {
	...
	public SaleRowDataGateway(ResultSet rs) throws RecordNotFoundException {
		try {
			fillAttributes(rs.getDate("date"), rs.getInt("customer_vat"), rs.getString("status"), rs.getDouble("total"));
	...
	private void fillAttributes(Date date, int customerVat, String status, double total) {
	 ...
	 * Remove sale part
	 */
	private static final String REMOVE_SALE_BY_VAT = "DELETE FROM sale where customer_vat = ?";
	
	public static void removeSaleByVat (int vat) throws PersistenceException {
		try (PreparedStatement statement = DataSource.INSTANCE.prepare(REMOVE_SALE_BY_VAT)){
			// set statement arguments
			statement.setInt(1, vat);
			//execute SQL
			statement.executeUpdate();
		} catch (SQLException e) {
			throw new PersistenceException("Internal error removing sales from " + vat + ".", e);
		}
	}
