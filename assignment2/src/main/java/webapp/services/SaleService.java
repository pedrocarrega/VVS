...
public enum SaleService {
	...
	public void addSale(int customerVat) throws ApplicationException {
		try {
			...
			throw new ApplicationException ("Can't add sale for customer with vat number " + customerVat + ".", e);
		}
	}
	...
	public int addSaleDelivery(int sale_id, int addr_id) throws ApplicationException {
		try {
			if(AddressRowDataGateway.getAddress(addr_id) == null)
				throw new ApplicationException("Address doesnt exist.");
			SaleRowDataGateway s = SaleRowDataGateway.getSaleById(sale_id);
			if(s.getStatus().equals(SaleStatus.CLOSED))
				throw new ApplicationException("Sale is already closed.");
