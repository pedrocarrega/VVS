...
public enum CustomerService {
	...
	public void addAddressToCustomer(int customerVat, String addr) throws ApplicationException {
		if (!isValidVAT (customerVat))
			...
			if(AddressRowDataGateway.addressExists(customerVat, addr))
				throw new ApplicationException("Address already exists on the given customer " + addr);
	...
	public void removeCustomer(int vat) throws ApplicationException {
		if (!isValidVAT (vat))
			...
			AddressRowDataGateway.removeAddresses(vat);
			SaleRowDataGateway.removeSaleByVat(vat);
			SaleDeliveryRowDataGateway.removeDeliveryByVat(vat);			
		} catch (PersistenceException e) {
